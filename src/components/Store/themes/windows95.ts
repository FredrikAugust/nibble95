...
const Store = css<ThemeProps<StoreProps>>`
    ...
    grid-row: 1 /span 2;
    ${(props) => `${props.theme.windowActivity === ApplicationWindowTypes.FOCUSED ? 'z-index: 1;' : 'z-index: 0;'}`}
    ${(props) => `${props.theme.windowActivity === ApplicationWindowTypes.MINIMIZED ? 'display: none;' : ''}`}
    ${(props) => `${props.theme.user ? 'grid-column: 1 /span 2;' : 'grid-column: 2;'}`}
`;
